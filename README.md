...
### พยากรณ์แบบไม่บันทึกข้อมูล (หน้า `/new-forecast`)

อีกทางเลือกหนึ่งสำหรับดูผลพยากรณ์ของบริษัทที่ **ยังไม่มีอยู่ในทะเบียนลูกค้าเลย** (ไม่ต้อง
เพิ่มลง `customers.csv` หรือ `customers_local.csv` ก่อน) — หน้านี้ให้พิมพ์ชื่อบริษัท +
เลือกประเภทธุรกิจ/อัตรา + KVA (ไม่บังคับ) แล้วดูผลพยากรณ์ทันที

จุดสำคัญ: **ชื่อบริษัทที่พิมพ์ในฟอร์มนี้ไม่ถูกส่งไปที่เซิร์ฟเวอร์เลย** — ฝั่ง JavaScript
(`web/static/adhoc.js`) ส่งแค่ `business_type_code` / `rate_code` / `contract_kva` ไปที่
...
ดิสก์/log ที่ไหนทั้งสิ้น — ปิดแท็บแล้วข้อมูลหายไปทันที จึงพิมพ์ชื่อบริษัทจริงในหน้านี้ได้
